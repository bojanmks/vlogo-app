...
f=a.food.current_page;a=a.food.last_page;b=`<ul class='pagination font-small'><li class='page-item ${1===f?"disabled":""}'><a href='#' class='page-link pagination-back'><i class='fas fa-caret-left'></i></a></li>`;for(c=0;c<a;c++)b+=`<li class="page-item ${c+1===f?"active":""}"><a href='#' class="page-link number-link" data-page='${c+1}'>${c+1}</a></li>`;b+=`<li class='page-item ${f===a?"disabled":""}'><a href='#' class='page-link pagination-next'><i class='fas fa-caret-right'></i></a></li></ul>`;
$("#foodPagination").html(b);m();n()}function m(){$(".pagination a").click(function(a){a.preventDefault()});$(".page-link.number-link").click(function(){const a=$(".page-item.active .page-link").data("page"),b=$(this).data("page");b!==a&&e(b)});$(".pagination-back").click(function(){const a=$(".page-item.active .page-link").data("page");1>=a||e(a-1)});$(".pagination-next").click(function(){const a=$(".page-item.active .page-link").data("page"),b=$(".page-link").length;a>=b||e(a+1)})}function n(){$(".add-food").on("click",
async function(a){a.preventDefault();a=$(this).data("id");await ajaxCallback(base_url+"/cart/add","POST",{id:a})&&g()})}async function g(){var a=window.location.href.split("/");a=parseInt(a[a.length-1]);a=await ajaxCallback(base_url+"/cart/items","POST",{restaurantId:a});let b="",c=0;if(a.length){for(let d of a)c+=parseFloat(d.price)*parseInt(d.amount),b+=`<li class="order-item font-small row d-flex align-items-center justify-content-between">
